...
const MenuScreen = () => {
  ...
  const fetchMenu = async (selectedDate: Date) => {
    ...
    try {
      ...
      const url = `https://techdining.api.nutrislice.com/menu/api/weeks/school/${location}/menu-type/${meal}/${formattedDatePath}/?format=json`;
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.heading}>
        {meal?.toString().toUpperCase()} at {location?.toString().replace("-", " ").toUpperCase()}
      </Text>
      <Text style={{ fontSize: 16, marginBottom: 8 }}>for {date.toDateString()}</Text>
            ...
            item.food ? (
              <Pressable
                onPress={() => {
                  ...
                  router.push({
                    pathname: "../food/[id]",
                    params: { id: String(item.food.id), item: encodedItem },
                  });
                }}
              >
